...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
  ...
  $hashed_password = hash('sha512', $_POST['password']) ?? null;
  ...
  $sql = "INSERT INTO user(email,password,firstname,lastname,phonenumber,street,streetnumber,postalcode,locality,region,country) VALUES (\"$email\", \"$hashed_password\", \"$firstname\", \"$lastname\", \"$phonenumber\", \"$street\", \"$streetnumber\", \"$postalcode\", \"$locality\", \"$region\", \"$country\" )";
  ...
  <div class="card cardregister-container">
    <form method="POST" class="form-register">
      <div class="register-left">
        <input type="email" name="email" id="email" class="form-control" value="<?php echo htmlentities($_POST['email'] ?? '') ?>" placeholder="Email address" required autofocus>
      ...
      <div class="register-right">
        ...
        <br /><br /><button class="btn btn-lg btn-primary btn-block btn-signin btn-register" type="submit">Register</button>
      </div>
    </form>
